...
LISTA_L_UNIDIRECIONAL = LINGUAS_UNIDIRECIONAIS.keys()
...
IDIOMAS = {
    ...
    "Swedish (sv)": "sv",
    "Amharic (am)": "am",
    "Welsh (cy)": "cy",  # sem aux gTTS
    "Estonian (et)": "et",
    "Croatian (hr)": "hr",
    "Icelandic (is)": "is",
    "Georgian (ka)": "ka",  # sem aux gTTS
    "Khmer (km)": "km",
    "Slovak (sk)": "sk",
    "Albanian (sq)": "sq",
    "Serbian (sr)": "sr",
    "Azerbaijani (az)": "az",  # sem aux gTTS
    "Bulgarian (bg)": "bg",
    "Galician (gl)": "gl",  # sem aux gTTS
    "Gujarati (gu)": "gu",
    "Kazakh (kk)": "kk",  # sem aux gTTS
    "Kannada (kn)": "kn",
    "Lithuanian (lt)": "lt",  # sem aux gTTS
    "Latvian (lv)": "lv",
    "Macedonian (mk)": "mk",  # sem aux gTTS # erro ao obter modelo de alinhamento
    "Malayalam (ml)": "ml",
    "Malay (ms)": "ms",  # erro ao obter alinhamento do modelo
    "Romanian (ro)": "ro",
    "Sinhala (si)": "si",
    "Sundanese (su)": "su",
    "Swahili (sw)": "sw",  # erro
...
LISTA_BASE_L = IDIOMAS.keys()
LISTA_DE_IDIOMAS = [list(LISTA_BASE_L)[0]] + sorted(list(LISTA_BASE_L )[1:])
LINGUAS_INVERTIDAS = {value: key for key, value in IDIOMAS.items()}
...
ALINHAMENTO_EXTRA = {
    ...
    "as": "",
    "eu": "", # cahya/wav2vec2-large-xlsr-basque # verificar
    "ha": "infinitejoy/wav2vec2-large-xls-r-300m-hausa",
    "ht": "",
    "hy": "infinitejoy/wav2vec2-large-xls-r-300m-armenian", # sem (.)
    "lo": "",
    "mg": "",
...
def corrigir_linguagem_de_codigo(translate_to, syntax="google"):
    if syntax == "google":
        # google-translator, gTTS
        substituir_codigo_de_idioma = {"zh": "zh-CN", "he": "iw", "zh-cn": "zh-CN"}
    elif syntax == "coqui":
        # coqui-xtts
        substituir_codigo_de_idioma = {"zh": "zh-cn", "zh-CN": "zh-cn", "zh-TW": "zh-cn"}

    nova_linguagem_de_codigo = substituir_codigo_de_idioma.get(translate_to, translate_to)
    logger.debug(f"Fix code {translate_to} -> {nova_linguagem_de_codigo}")
    return nova_linguagem_de_codigo
